                ...
                <div class="well">
                    ...
                    <form method="post" action="{{route('yonetim.kategori')}}" class="form-inline">
                        ...
                        <div class="form-group">
                            ...
                            {{--Üst Kategori kısmı burada gösterilmiş oldu--}}
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        @endif
                           ...
                           @foreach($list as $gelen)
                            <tr>
                                <td>{{ $gelen->id }}</td>
                                <td>{{ $gelen->ust_kategori->kategori_adi}}</td>
                                <td>{{ $gelen->slug}}</td>
                                <td>{{ $gelen->kategori_adi }}</td>
                                <td>{{ $gelen->olusturulma_tarihi }}</td>
                                <td style="width: 100px">
                                    <a href="{{ route('yonetim.kategori.duzenle',$gelen->id) }}" class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="top" title="Düzenle">
                                        <span class="fa fa-pencil"></span>
                                    </a>
                                    <a href="{{route('yonetim.kategori.sil' ,$gelen->id)}}" class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="top" title="Sil" onclick="return confirm('Emin Misiniz?')">
